...
import { 
  FilePlus2, FileText, LayoutTemplate, Settings, User, ArrowRight, UploadCloud, 
  X, File, UserPlus, MapPin, Send, Trash2 
} from 'lucide-react';
...
function UploadFlowModal({ onClose }) {
  ...
  const [signers, setSigners] = useState([{ id: uuidv4(), name: '', email: '', order: 0 }]);
  ...
  const validateStep = (stepNumber) => {
    switch(stepNumber) {
      ...
      case 2: return signers.every(s => s.name.trim() && s.email.trim());
      case 3: return true; // Field placement will be optional for now
      case 4: return documentTitle.trim() !== '';
  ...
  const handleSendDocument = async () => {
    try {
      ...
      const documentId = await DocumentService.createDocument(documentData, userId);

      // Add default signature fields for each signer
      // In a real implementation, this would be done in Step 3
      for (const signer of signers) {
        await DocumentService.addField(documentId, {
          type: 'signature',
          x: 100,
          y: 500,
          width: 200,
      ...
      // Send for signature
      await DocumentService.sendForSignature(documentId, userId);

      alert('Document sent successfully! (In Phase 1, actual email sending will be implemented)');
      onClose();
  ...
  const steps = [
    { number: 1, title: 'Upload Document', icon: File },
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl h-full max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <header className="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 className="text-xl font-semibold text-gray-800">New Signature Request</h2>
            ...
            <ol className="space-y-4">
              {steps.map(s => (
                <li key={s.number} className="flex items-center">
                  <span className={`flex items-center justify-center w-8 h-8 rounded-full font-bold text-sm ${step >= s.number ? 'bg-indigo-600 text-white' : 'bg-gray-200 text-gray-600'}`}>
                    {s.number}
                  </span>
                  <span className={`ml-3 text-sm font-medium ${step >= s.number ? 'text-gray-900' : 'text-gray-500'}`}>{s.title}</span>
                </li>
              ))}
            </ol>
          </div>
        ...
        <footer className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50 rounded-b-2xl">
          ...
          >
            {uploading ? 'Sending...' : (step === 4 ? 'Send Document' : 'Next')}
          </button>
        </footer>
      </div>
    </div>
...
function Step1Upload({ file, setFile, onFileChange, onFileDrop, documentTitle, setDocumentTitle }) {
  return (
    <div onDragOver={(e) => e.preventDefault()} onDrop={onFileDrop}>
      ...
      <p className="text-sm text-gray-500 mb-6">File will be converted to a secure, non-editable PDF.</p>
      ...
      <div className="w-full p-8 border-2 border-dashed border-gray-300 rounded-lg text-center cursor-pointer hover:border-indigo-500 bg-gray-50 transition-colors">
        <input type="file" id="file-upload" className="hidden" onChange={onFileChange} accept=".pdf,.doc,.docx,.png,.jpg" />
        <label htmlFor="file-upload" className="cursor-pointer">
          <UploadCloud className="w-12 h-12 mx-auto text-gray-400" />
          <p className="mt-4 text-sm font-semibold text-gray-700">
            <span className="text-indigo-600">Click to upload</span> or drag and drop
          </p>
          <p className="mt-1 text-xs text-gray-500">PDF, DOC, DOCX, PNG, JPG (up to 25MB)</p>
        </label>
      </div>
      ...
      {file && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between">
          ...
          <button onClick={() => setFile(null)} className="p-1 rounded-full hover:bg-green-100">
...
function Step2Recipients({ signers, addSigner, removeSigner, updateSigner }) {
  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-800 mb-2">Add recipients</h3>
      <p className="text-sm text-gray-500 mb-6">Specify who needs to sign the document.</p>
      
      <div className="space-y-4">
        {signers.map((signer, index) => (
          <div key={signer.id} className="border border-gray-200 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <label className="text-sm font-medium text-gray-700">Signer {index + 1}</label>
              {signers.length > 1 && (
                <button 
...
function Step3PlaceFields({ fileName }) {
  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-800 mb-2">Place signature fields</h3>
      <p className="text-sm text-gray-500 mb-6">For Phase 1, signature fields will be automatically placed. Advanced field placement coming in Phase 2.</p>
      
      <div className="w-full h-96 bg-gray-200 rounded-lg flex items-center justify-center">
        <div className="text-center p-4">
          <FileText className="w-16 h-16 text-gray-400 mx-auto" />
          <p className="mt-2 font-medium text-gray-700">{fileName || "document.pdf"}</p>
          <p className="mt-2 text-sm text-gray-500">Document preview and drag-and-drop field placement will be available in Phase 2.</p>
          <p className="mt-2 text-xs text-gray-400">For now, a signature field will be automatically added for each signer.</p>
        </div>
      </div>
    </div>
  );
...
function Step4Review({ documentTitle, signers, message, setMessage, file }) {
  return (
    <div>
      <h3 className="text-lg font-semibold text-gray-800 mb-2">Review & Send</h3>
      <p className="text-sm text-gray-500 mb-6">Review the details below before sending your document.</p>
      ...
      <div className="space-y-4 p-4 border rounded-lg bg-gray-50">
        ...
        <div>
          <label htmlFor="message" className="font-semibold text-gray-700">Message to Recipients:</label>
          <textarea 
            id="message" 
            rows="3" 
            value={message}
