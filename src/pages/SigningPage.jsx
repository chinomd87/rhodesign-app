...
const SigningPage = () => {
  ...
  const loadDocumentForSigning = async () => {
    try {
      setLoading(true);
      const validation = await SigningService.validateSigningLink(documentId, signerId);
      ...
      const signerFields = validation.document.fields.filter(f => f.signerId === signerId);
      const initialValues = {};
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-2">Unable to Sign Document</h1>
        ...
        <div className="max-w-4xl mx-auto px-4 py-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-1">{document?.title || document?.originalFileName}</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="w-full bg-gray-100 rounded-lg border-2 border-dashed border-gray-300 p-8">
                <div className="text-center">
                  ...
                  <p className="text-gray-600 mb-2">Document: {document?.originalFileName}</p>
                  <p className="text-sm text-gray-500">PDF viewer and field placement will appear here</p>
                  ...
                  {signerFields.map((field, index) => (
                    <div key={field.id} className="mt-4">
                      ...
                      >
                        {field.type === 'signature' && <User className="w-4 h-4 mr-2" />}
                        {field.type === 'date' && <Calendar className="w-4 h-4 mr-2" />}
                        {field.type === 'text' && <MapPin className="w-4 h-4 mr-2" />}
                        {field.type === 'initial' && <User className="w-4 h-4 mr-2" />}
                        
                        {fieldValues[field.id] ? `${field.type} (completed)` : `Click to ${field.type}`}
                      </button>
                    </div>
                  ))}
                </div>
                ...
                {signerFields.map((field, index) => (
                  <div key={field.id} className="border border-gray-200 rounded-lg p-3">
                    ...
                    {field.type === 'signature' ? (
                      ...
                      >
                        {fieldValues[field.id] ? 'Update Signature' : 'Add Signature'}
                      </button>
                    ) : field.type === 'date' ? (
                      <input
                        type="date"
